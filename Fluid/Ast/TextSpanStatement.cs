...
namespace Fluid.Ast
{
    public sealed class OriginTextSpanStatement : Statement
    {
        ...
        public OriginTextSpanStatement(in TextSpan text)
        {
            _text = text;
        }

        public OriginTextSpanStatement(string text)
        {
            ...
            if (!_isStripped)
            {
                ...
                lock (_synLock)
                {
                    if (!_isStripped)
                    {
                        ...
                        //"".Substring(0,19)
                        //_buffer = _text.Buffer.Substring(_text.Offset + start, end - start + 1);
    ...
    public sealed class TextSpanStatement : Statement
    {
        ...
        public override ValueTask<Completion> WriteToAsync(TextWriter writer, TextEncoder encoder, TemplateContext context)
        {
            ...
            writer.Write(_text.Buffer.AsSpan(_text.Offset, _text.Length));
#else
            static async ValueTask<Completion> Awaited(Task task)
            {
                await task;
